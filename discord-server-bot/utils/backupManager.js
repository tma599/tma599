...
async function executeBackup({ guild, userId, backupName, isAuto = false, backupMessages = true }) {
  let sanitizedName;
  if (isAuto) {
    const now = new Date();
    ...
    sanitizedName = backupName.replace(/[^a-zA-Z0-9_\-぀-ゟ゠-ヿ一-龯]+/g, '');
    if (!sanitizedName) {
      throw new Error(
        '無効なバックアップ名です。英数字、アンダースコア(_)、ハイフン(-)、日本語のみ使用できます。'
      );
    }
  }
  ...
  if (backupMessages) {
    ...
    console.log('[バックアップ] メッセージ履歴のバックアップはスキップされました。');
    // メッセージ履歴なしでチャンネル情報のみ収集
  ...
  // Ban情報の収集
  console.log('[バックアップ] Ban情報を収集しています...');
  try {
    ...
    console.error('[エラー] Ban情報の収集に失敗しました。', error);
  }
  console.log('[バックアップ] Ban情報の収集が完了しました。');

  // 絵文字・ステッカー情報の収集
  console.log('[バックアップ] 絵文字・ステッカー情報を収集しています...');
  ...
  console.log('[バックアップ] 絵文字・ステッカー情報の収集が完了しました。');

  // 招待リンク情報の収集
  console.log('[バックアップ] 招待リンク情報を収集しています...');
  try {
    ...
    console.error('[エラー] 招待リンク情報の収集に失敗しました。', error);
  }
  console.log('[バックアップ] 招待リンク情報の収集が完了しました。');

  // AutoModerationルール情報の収集
  console.log('[バックアップ] AutoModerationルール情報を収集しています...');
  try {
    const autoModRules = await guild.autoModerationRules.fetch();
    backupData.autoModRules = autoModRules.map((rule) => ({
      name: rule.name,
      id: rule.id,
    ...
    console.error('[エラー] AutoModerationルール情報の収集に失敗しました。', error);
  }
  console.log('[バックアップ] AutoModerationルール情報の収集が完了しました。');

  // スケジュールイベント情報の収集
  console.log('[バックアップ] スケジュールイベント情報を収集しています...');
  try {
    const scheduledEvents = await guild.scheduledEvents.fetch();
    backupData.scheduledEvents = scheduledEvents.map((event) => ({
      name: event.name,
      id: event.id,
    ...
    console.error('[エラー] スケジュールイベント情報の収集に失敗しました。', error);
  }
  console.log('[バックアップ] スケジュールイベント情報の収集が完了しました。');
...
module.exports = { getBackupDir, ensureBackupDir, executeBackup };
